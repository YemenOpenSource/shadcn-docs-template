...
`NavigationItem` is defined once in `app/lib/navigation.ts` and shared across the content tree, `DocsSidebar`, `MobileNav`, and `CommandMenu` — no per-component redefinitions.
